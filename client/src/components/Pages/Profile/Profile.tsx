...
import { changeName, logout, uploadAvatar } from "../../../slices/user.slice";
...
const Profile = () => {
  const { username, id, avatar } = useAuth();
  ...
  const handleUpload = async () => {
    if (!selectedAvatar) {
      notify(" X You not select avatar");
      return;
    }
    const avatar = new FormData();
    avatar.append("avatar", selectedAvatar);
    const response = await AppService.uploadAvatar(id, avatar);
    dispatch(uploadAvatar(response.data.filename));
    notify("✔ Your avatar has successfully uploaded");
  };
    ...
    <>
      ...
      <styled.Profile>
        ...
        <div className="avatar">
          ...
          {avatar && <img src={`/avatars/${avatar}`} alt="" />}
        </div>
      </styled.Profile>
    </>
  );
